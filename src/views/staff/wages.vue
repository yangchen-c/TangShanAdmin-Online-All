  ...
  <div id="app">
    ...
    <div class="tablee">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column align="center" prop="staff.name" label="员工名称" />
        <el-table-column align="center" prop="staff.salary" label="底薪" />
        <el-table-column align="center" prop="commission" label="提成" />
        <el-table-column align="center" prop="createTime" label="结算时间" />
    ...
    <el-dialog title="编辑工资" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        <el-form-item label="工资调整" :label-width="formLabelWidth">
          <el-input v-model="form.staff.salary" placeholder="请输入工资" style="width:400px" />
        </el-form-item>
        <el-form-item label="提成调整" :label-width="formLabelWidth">
          <el-input v-model="form.commission" placeholder="请输入工资" style="width:400px" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 提成
    getList() {
      salaryList({ id: this.$route.params.id })
        .then((response) => {
          this.tableData = response.data.data
          // this.$route.query.id = ''
        })
    ...
    getEditData(data) {
      ...
      this.form.staff.id = data.staff.id
      this.form.commission = data.commission
      this.form.staff.salary = data.staff.salary
    },
    // 编辑新增确定事件
    addSubmit() {
      // this.btnLoading = true
      if (this.form.id) {
        salaryUpdate(this.form)
          .then(() => {
            this.$notify.success({
              title: '成功',
              message: '员工修改成功'
