<template>
  <div id="app">
    <div class="btn">
      ...
      <el-select v-model="value" clearable placeholder="选择门店" @change="changeShop($event)">
    ...
    <el-dialog :title="title1" :visible.sync="dialogFormVisible" width="65%">
      <el-form :model="form" :inline="true">
        ...
        <el-form-item label="所属门店">
          <el-select v-model="form.store.id" clearable placeholder="请选择">
            <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    changeData(val) {
      console.log(val)
      if (val !== null) {
        this.tableData = this.tableDataAll.filter((el) => el.shootTime === val)
      } else {
